...
int readWav_float32(const char* path, std::vector<double>* amplitudeData) {
    ...
    while (av_read_frame(formatContext, packet) >= 0) {
        if (packet->stream_index == audioStreamIndex) {
            if (avcodec_send_packet(codecContext, packet) == 0) {
                while (avcodec_receive_frame(codecContext, frame) == 0) {
                    // Check if the audio is in AV_SAMPLE_FMT_FLT format
                    if (codecContext->sample_fmt == AV_SAMPLE_FMT_FLT) {
                        int data_size = av_get_bytes_per_sample(codecContext->sample_fmt);
                        for (int i = 0; i < frame->nb_samples; i++) {
                            for (int ch = 0; ch < codecContext->ch_layout.nb_channels; ch++) {
                                if (frame->data[ch] != nullptr) {
                                    float* sample = (float*)(frame->data[ch] + data_size * i);
                                    amplitudeData->push_back((double)(*sample));
                                }
                            }
                        }
                    }
